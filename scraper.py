...
def normalize_url(url: str) -> str:
    try:
        ...
        host = netloc
        if any(host.endswith(d) for d in ["globalcropprotection.com", "noticiasagricolas.com.br", "agrolink.com.br"]):
...
def fetch_dashboard_articles() -> List[Article]:
    ...
    results: Dict[str, List[Article]] = {"na": [], "gcp": [], "ag": []}

    def run_na():
        try:
            na = NoticiasAgricolasScraper()
            results["na"] = na.collect_dashboard_articles(limit=15)
        except Exception:
            results["na"] = []
    ...
    def run_gcp():
        try:
            ...
            results["gcp"] = gcp.collect_recent_articles(limit=15)
        except Exception:
            results["gcp"] = []
    ...
    def run_ag():
        try:
            ag = AgrolinkScraper()
            results["ag"] = ag.collect_recent_articles(limit=15)
        except Exception:
            results["ag"] = []

    threads = [threading.Thread(target=run_na), threading.Thread(target=run_gcp), threading.Thread(target=run_ag)]
    for t in threads:
        t.daemon = True
        t.start()
    for t in threads:
...
class AgrolinkScraper:
    ...
    def _parse_listing(self, max_candidates: int = 50) -> List[Dict[str, object]]:
        ...
        for base in self.BASES:
            try:
                resp = self.session.get(base, timeout=REQUEST_TIMEOUT)
                resp.raise_for_status()
            except Exception:
                continue
            soup = BeautifulSoup(resp.text, "lxml")
...
def fetch_quotes() -> List[Quote]:
    ...
    items: List[Quote] = []
    try:
        r = session.get(f"{BASE_URL}/cotacoes/", timeout=REQUEST_TIMEOUT)
        ...
        try:
            rc = session.get(f"{BASE_URL}/cotacoes/cafe", timeout=REQUEST_TIMEOUT)
        ...
        if not m_val_quote.value:
            try:
                rm = session.get(f"{BASE_URL}/cotacoes/milho", timeout=REQUEST_TIMEOUT)
        ...
        if not s_val_quote.value:
            try:
                rs = session.get(f"{BASE_URL}/cotacoes/soja", timeout=REQUEST_TIMEOUT)
